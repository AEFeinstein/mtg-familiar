...
public class CardHelpers {
    ...
    public static String cardListToDec(List<? extends MtgCard> mtgCards) {
        ...
        for (MtgCard card : mtgCards) {
            if (!"".equals(card.mName)) {
                if (card instanceof CompressedDecklistInfo &&
                        ((CompressedDecklistInfo)card).mIsSideboard) {
                    deck.append("SB: ")
                ...
                deck.append(card instanceof CompressedCardInfo ?
                        ((CompressedCardInfo)card).getTotalNumber() : card.numberOf)
                        .append(' ')
                        .append(card.mName)
                        .append('\n');
            }
